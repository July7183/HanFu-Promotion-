...
<div class="container">
	...
	<ul role="tablist" class="nav nav-tabs">
        ...
        <li role="presentation" <c:if test="${status==5}">class="active"</c:if>><a href="indentList.action?status=5">已取消</a></li>
        
    </ul>
	
	<br>
	
		<form class="form-inline" action="indentSearch.action" id="serchform" method="post">
		<div class="form-group">
			<label for="id">ID: </label>
			<input type="text" class="form-control" id="id" name="indent.id">
		</div>
		...
		<c:forEach var="indent" items="${indentList}">
	         <tr>
	         	...
	         	<td>
	         		...
	         		<c:if test="${indent.status==5}">已取消</c:if>
	         	</td>
				...
				<td>
					...
					<c:if test="${indent.status==5}"><a class="btn btn-danger" href="indentDelete?id=${indent.id}&status=${status}&page=${page}">删除</a></c:if>
