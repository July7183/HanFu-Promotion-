...
<div class="container">
	...
	<ul role="tablist" class="nav nav-tabs">
        <li role="presentation" <c:if test="${status==0}">class="active"</c:if>><a href="productList.action">全部商品</a></li>
        <li role="presentation" <c:if test="${status==1}">class="active"</c:if>><a href="productList.action?status=1">精品推荐</a></li>
        <li role="presentation" <c:if test="${status==2}">class="active"</c:if>><a href="productList.action?status=2">促销活动</a></li>
        <li role="presentation" <c:if test="${status==3}">class="active"</c:if>><a href="productList.action?status=3">新品推荐</a></li>
    </ul>
		...
		<c:forEach var="product" items="${productList}">
	         <tr>
				...
				<td>
					...
					<c:if test="${!product.isNew}">
						<a class="btn btn-info" href="productNewA.action?id=${product.id}&page=${page}&status=${status}">设为新品</a>
					</c:if>
					<br><br>
