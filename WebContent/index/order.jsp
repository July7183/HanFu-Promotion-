						...
						<tr>
		          			<td>
			          			...
			          			<s:if test="status==1">(未付款)
			          			        <a href="topay.action?indentid=${id}"><button>付款</button></a>
			          			        <a href="indentCancel.action?indentid=${id}" onclick="return confirm('您确定要进行取消订单操作？');"><button>取消订单</button></a>
			          			</s:if>
			          			<s:if test="status==2">(已付款)</s:if>
			          			<s:if test="status==3">(已发货)
			          			        <a href="indentFinish.action?indentid=${id}" onclick="return confirm('您确定要进行确认收货操作？');"><button>确认收货</button></a>
			          			</s:if>
			          			<s:if test="status==4">(已完成)</s:if>	
			          			<s:if test="status==5">(已取消)</s:if>			          
		          			</td>
