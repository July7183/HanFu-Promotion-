...
@Results({
	...
	@Result(name="reorder",type="redirect",location="order.action"),
...
public class UserAction extends BaseAction{
	...
	public String register(){
		if (user.getUsername().isEmpty()||user.getPayword().isEmpty()||user.getPassword().isEmpty()) {
			addActionMessage("请注意，前三项不能为空!");
			return "register";
	 ...
	 */
	@Action("indentCancel")
	public String indentCancel() {
		Users user = (Users) getSession().getAttribute("user");
		if (user == null) {
			addActionError("请先登录!");
			return "login";
		}
		indentService.cancel(indentid);
