...
@Results({
	...
	@Result(name="news", location="/admin/news_list.jsp"),
	@Result(name="newsadd", location="/admin/news_add.jsp"),
	@Result(name="newsedit", location="/admin/news_edit.jsp"),
	@Result(name="renews", type="redirect", location="newsList.action"),
	@Result(name="knowledge", location="/admin/knowledge_list.jsp"),
	@Result(name="knowledgeadd", location="/admin/knowledge_add.jsp"),
	@Result(name="knowledgeedit", location="/admin/knowledge_edit.jsp"),
	@Result(name="reknowledge", type="redirect", location="knowledgeList.action"),
	@Result(name="essay", location="/admin/essay_list.jsp"),
	@Result(name="essayadd", location="/admin/essay_add.jsp"),
	@Result(name="essayedit", location="/admin/essay_edit.jsp"),
	@Result(name="reessay", type="redirect", location="essayList.action"),
	@Result(name="carousel", location="/admin/carousel_list.jsp"),
	@Result(name="recarousel", type="redirect", location="carouselList.action"),
})		
public class AdminAction extends BaseAction{
     ...
     */
    @Action("indentSearch")
	public String indentSearch(){
    	flag = 1;
		if (indent!=null && indent.getId() > 0) {
			indentList = Arrays.asList(indentService.get(indent.getId()));
		}else {
			return indentList();
		}
		return "indent";
	...
	public String newsList(){
    	flag = 5;
		newsList = newsService.getNewsList(page,rows);
		pageHtml = PageUtil.getPageToolAdmin(request, newsService.getNewsTotal(), page, rows);
		return "news";
	}
	...
	public String newsSearch(){
		if (news!=null && news.getId() > 0) {
			newsList = Arrays.asList(newsService.get(news.getId()));
			return "news";
		}else {
			return newsList();
		}
	...
	public String update(){
    	News newsdb = newsService.get(news.getId());
    	if (null != photo) {
    		newsdb.setImag(UploadUtil.fileUpload(photo, photoFileName, "picture"));
    	}
    	newsdb.setContent(news.getContent());
    	newsdb.setTitle(news.getTitle());
		if(newsService.updateNews(newsdb)){
	...
	@Action("newsSave")
	public String save(){
		news.setImag(UploadUtil.fileUpload(photo, photoFileName, "picture"));
		...
		public String knowledgeList(){
	    	flag = 6;
			setKnowledgeList(knowledgeService.getKnowledgeList(page,rows));
			pageHtml = PageUtil.getPageToolAdmin(request, knowledgeService.getKnowledgeTotal(), page, rows);
			return "knowledge";
		}
		...
		public String knowledgeSearch(){
			if (knowledge!=null && knowledge.getId() > 0) {
				knowledgeList = Arrays.asList(knowledgeService.get(knowledge.getId()));
				return "knowledge";
			}else {
		 ...
		 * @return
		 */
		@Action("knowledgeEdit")
		public String knowledgeedit(){
			flag = 6;
		...
		public String update2(){
	    	...
	    	Knowledge knowledgedb = knowledgeService.get(knowledge.getId());
	    	if (null != photo) {
	    		knowledgedb.setImag(UploadUtil.fileUpload(photo, photoFileName,  "picture"));
	    	}
	    	knowledgedb.setContent(knowledge.getContent());
	    	knowledgedb.setTitle(knowledge.getTitle());
	    	
			if(knowledgeService.updateKnowledge(knowledgedb)){
				request.setAttribute("msg", "更新成功!");
			}
			else{
				request.setAttribute("msg", "更新失败!");
		...
		public String save2(){
			knowledge.setImag(UploadUtil.fileUpload(photo, photoFileName, "picture"));
		...
		public String essayList(){
	    	flag = 7;
			setEssayList(essayService.getEssayList(page,rows));
			pageHtml = PageUtil.getPageToolAdmin(request, essayService.getEssayTotal(), page, rows);
			return "essay";
		}
	     ...
	     */
	    @Action("essaySearch")
		public String essaySearch(){
			if (essay!=null && essay.getId() > 0) {
				essayList = Arrays.asList(essayService.get(essay.getId()));
				return "essay";
			}else {
				return essayList();
			}
		...
		public String update3(){
	    	Essay essaydb = essayService.get(essay.getId());
	    	if (null != photo) {
	    		essaydb.setImag(UploadUtil.fileUpload(photo, photoFileName,  "picture"));
	    	}
	    	essaydb.setContent(essay.getContent());
	    	essaydb.setTitle(essay.getTitle());
			if(essayService.updateEssay(essaydb)){
				request.setAttribute("msg", "更新成功!");
			}
			else{
				request.setAttribute("msg", "更新失败!");
		...
		public String delete3(){
			essayService.deleteEssay(essay);
			return "reessay";
		}
		/**
		 * 文章保存
		...
		@Action("essaySave")
		public String save3(){
			essay.setImag(UploadUtil.fileUpload(photo, photoFileName, "picture"));
	...
	public String adminDelete(){
		...
		public String carouselList(){
	    	flag = 10;
			setCarouselList(carouselService.getCarouselList(1,6));
			return "carousel";
		}
		 ...
		 * @return
		 */
	    @Action("carouselUpdate")
		public String update4(){
	    	Carousel carouseldb = carouselService.get(id);
	    	if (photo != null) {
	    		carouseldb.setImag(UploadUtil.fileUpload(photo, photoFileName,  "picture"));
	    		
	    		if(carouselService.updateCarousel(carouseldb)){
	    			request.setAttribute("msg", "更新成功!");
	    		}
	    	}
			else{
				request.setAttribute("msg", "请选择图片进行更新");
			}
			return "recarousel";
		}
	    	/**
		...
		public String delete4(){
			carouselService.deleteCarousel(carouselService.get(id));
			return "recarousel";
		}
